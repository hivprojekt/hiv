...
#if defined(NDEBUG)
# error "HIVcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6cc57ddfe3d6798ff7ee7596d87ef769f1e25f211cf0be4e38f4864079550f5d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HIVcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HIVcoin Signed Message:\n";
...
{
    // HIVcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 9 * 60; // HIVcoin: 9 minutes
static const int64 nTargetSpacing = 4 * 60; // HIVcoin: 4 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HIVcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // HIVcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6cc57ddfe3d6798ff7ee7596d87ef769f1e25f211cf0be4e38f4864079550f5d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "HIV is evil";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042133710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1504995000;
        ...
        assert(block.hashMerkleRoot == uint256("0xce5b0d410a5a85251acad76b5235e13c5e8f374735037026c0d1f916a825fa43"));
...
unsigned char pchMessageStart[4] = { 0xbb, 0xb0, 0x56, 0xdb }; // HIVcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HIVcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HIVcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HIVcoinMiner(CWallet *pwallet)
{
    printf("HIVcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("HIVcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HIVcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HIVcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HIVcoinMiner, pwallet));
}
